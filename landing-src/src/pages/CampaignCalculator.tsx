    ...
    <PageShell maxWidth="max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
        {[
          ...
          <div key={s.label} className="bg-white border border-gray-200 rounded-2xl p-4">
            ...
            <p className="text-[16px] font-medium text-gray-900">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[14px] font-medium text-gray-900">توزيع القنوات / Channel Split</h3>
          <button
            onClick={normalize}
          ...
          {rows.map((row, i) => (
            <div key={row.name} className="border border-gray-100 rounded-xl p-4">
              ...
              <div className="bg-gray-100 rounded-full h-1.5 overflow-hidden">
                <div className="bg-blue-500 h-full rounded-full transition-all duration-200" style={{ width: `${Math.min(100, row.pct)}%` }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-5">
        <h3 className="text-[14px] font-medium text-gray-900 mb-4">جدول الصرف الأسبوعي / Weekly Pacing</h3>
        <div className="space-y-2">
          {weeks.map((w) => (
            <div key={w.week} className="flex items-center justify-between text-[12.5px] py-1.5 border-b border-gray-100 last:border-0">
              <span className="text-gray-700">الأسبوع {w.week} ({w.days} يوم)</span>
              <span className="text-gray-500">{formatNumber(w.budget)} SAR</span>
              <span className="text-gray-400">تراكمي: {formatNumber(w.cumulative)} SAR</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-5">
        <h3 className="text-[14px] font-medium text-gray-900 mb-4">حفظ الخطة / Save Plan</h3>
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 rounded-xl px-4 py-3 border border-gray-200 text-[16px] sm:text-[13px] focus:outline-none focus:border-blue-400"
          />
          <button
            onClick={savePlan}
            disabled={saving}
            className="inline-flex items-center justify-center gap-2 text-[13px] font-medium text-white bg-blue-500 rounded-xl px-6 py-2.5 hover:bg-blue-600 transition-colors duration-200 disabled:opacity-60"
          >
      ...
      {plans.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <h3 className="text-[14px] font-medium text-gray-900 mb-4">الخطط المحفوظة / Saved Plans</h3>
          <div className="space-y-2">
            {plans.map((plan) => (
              <div key={plan.id} className="flex items-center justify-between text-[12.5px] py-2 border-b border-gray-100 last:border-0">
                <div className="min-w-0">
                  <p className="text-gray-800 font-medium truncate">{plan.name}</p>
